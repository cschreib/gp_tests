...
pos = mplotpos(multi=[2,1])

plot, p.x, p.y, xr=[min(p.xt), max(p.xt)], yr=[1.2*min(p.y), 1.2*max(p.y)], charsize=2, psym=5, $
    /nodata, pos=mplotpos(pos=0)

i = indgen(n_elements(p.xt))
s = sqrt(r.cov[i,i])
sf = sqrt(rf.cov[i,i])

oplot, p.x, p.y, psym=5, symsize=0.5, col=color_darken('ff00'x, 0.5)

oplot, p.xt, rf.m
oplot, p.xt, rf.m - 2*sf, line=2
oplot, p.xt, rf.m + 2*sf, line=2

oplot, p.xt, p.yt, col='ff'x

plot, p.x, p.y, xr=[min(p.xt), max(p.xt)], yr=[1.2*min(p.y), 1.2*max(p.y)], charsize=2, psym=5, $
    /nodata, pos=mplotpos(pos=1)
